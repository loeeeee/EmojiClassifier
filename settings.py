...
def init_log() -> None:
    try:
        log_path = os.path.join(os.environ["DATA_DIR"],".log")
        log_level = os.environ["LOG_LEVEL"].lower()
    except KeyError:
        print("[ERROR] Log: Cannot get env settings. Please check config.")
        raise KeyError
    ...
    logger.info("Logging is now in charge of logging.")
    return
...
def main() -> None:
    ...
    logger.info("Logger inalization complete.")
    return
